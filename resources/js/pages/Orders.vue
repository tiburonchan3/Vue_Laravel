        ...
        <div class="row mt-5">
          <div class="col-md-8">
            <nav aria-label="Page navigation">
              <ul class="pagination">
                <li
                  v-bind:class="{ disabled: !pag.first_link }"
                  class="page-item"
                  aria-current="page"
                >
                  <a
                    class="page-link"
                    href="#"
                    @click="GetResult(pag.first_link)"
                    >&laquo;</a
                  >
                </li>
                <li
                  v-bind:class="{ disabled: !pag.prev_link }"
                  class="page-item"
                  aria-current="page"
                >
                  <a
                    class="page-link"
                    href="#"
                    @click="GetResult(pag.prev_link)"
                    >Next</a
                  >
                </li>
                <li
                  v-for="n in pag.last_page"
                  :key="n"
                  v-bind:class="{ active: pag.current_page == n }"
                  class="page-item"
                  aria-current="page"
                >
                  <a
                    class="page-link"
                    href="#"
                    @click="GetResult(pag.path_page + n)"
                    >{{ n }}</a
                  >
                </li>
                <li
                  v-bind:class="{ disabled: !pag.next_link }"
                  class="page-item"
                  aria-current="page"
                >
                  <a
                    class="page-link"
                    href="#"
                    @click="GetResult(pag.next_link)"
                    >Prev</a
                  >
                </li>
                <li
                  v-bind:class="{ disabled: !pag.last_link }"
                  class="page-item"
                  aria-current="page"
                >
                  <a
          ...
          <div class="col-md-4 text-white">
            Pagina:{{ pag.from_page }} - {{ pag.to_page }} Total:
            {{ pag.total_pages }}
          </div>
        </div>
      </div>
